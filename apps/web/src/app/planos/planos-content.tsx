...
import { useSiteBranding, type SiteBranding } from "@/lib/branding";
 ...
 */
export function PlanosPageClient({ initialBranding }: { initialBranding: SiteBranding | null }) {
  return (
    <Suspense>
      <PlanosContent initialBranding={initialBranding} />
    </Suspense>
  );
}

function PlanosContent({ initialBranding }: { initialBranding: SiteBranding | null }) {
  const { t } = useI18n();
  ...
  const branding = useSiteBranding(initialBranding ?? undefined);
    ...
    <div className="min-h-screen bg-surface-alt">
      <header className="flex items-center justify-between px-6 py-4">
        <div className="flex items-center gap-2">
          ...
          />
          {!branding.isLoading && !branding.data?.tenantLoginLogo.logoLightUrl && !branding.data?.tenantLoginLogo.logoDarkUrl && (
